...
class HorizonPlayer extends Player
{
    ...
    public $duel_waiting = false;

    const COMBAT_TIMER = 15;

    public $has_claimed_kit_this_life = false;

    public $gamemode = Constants::GAMEMODE_SPAWN;
    ...
    public function isInKitPvPArea()
    {
        return !$this->isInPracticeZone() && $this->getLevel()->getName() === WorldMap::KIT;
    }
    ...
    public function setSpawnItems()
    {
        ...
        $duel_players = Main::getDuelManager()->getTotalDuelPlayers();
        $map = [
    ...
    {
        $this->getSession()->getScoreboard()->setScoreboard(new KitPvP($this));
        if ($bool) {
            if ($savedItems) {
                $this->saveInventory();
            }
            ...
            $this->gamemode = Constants::GAMEMODE_SPAWN;
        } else {
            $this->getInventory()->clearAll();
            $this->getArmorInventory()->clearAll();
            if ($savedItems) {
            ...
            $this->gamemode = Constants::GAMEMODE_KITPVP;
            $this->in_kitpvp = true;
        }
    }
    ...
    {
        $this->gamemode = Constants::GAMEMODE_SPAWN;
        $this->teleport($this->getServer()->getLevelByName(WorldMap::KIT)->getSpawnLocation());
    }
    ...
    {
        return $this->getSession()->getEconomy();
    }

    public function getExperience(): Experience
    {
        return $this->getSession()->getExperience();
    }

    public function getMember()
    {
        /** @var Hierarchy $plugin */
        $plugin = $this->getServer()->getPluginManager()->getPlugin("Hierarchy");
        return $plugin->getMemberFactory()->getMember($this);
    }

    public function getRank()
    {
        return $this->getMember()->getTopRole()->getName();
    }
